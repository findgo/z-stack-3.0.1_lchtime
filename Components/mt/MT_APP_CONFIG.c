 ...
 *          If IC is selected, the IC must be included in the buffer considering 
 *          the IC format used by the joining device, see BDB_INSTALL_CODE_USE
 *
 * @param   pBuf - pointer to received buffer
 *
 * @return  void
...
static void MT_AppCnfBDBSetActiveCentralizedKey(uint8* pBuf)
{
  ...
  if(*pBuf)
  {
    //Set Global Default Centralized key
    retValue = bdb_setActiveCentralizedLinkKey(TRUE,NULL);
  }
  else
  {
    pBuf++;
    retValue = bdb_setActiveCentralizedLinkKey(FALSE,pBuf);
  }
