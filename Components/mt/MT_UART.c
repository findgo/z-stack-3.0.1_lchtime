...
static uint16 msptempDataLen;
static void MSP_UartProcessZToolData ( uint8 port, uint8 event )
{
    uint8  ch;
    uint16  bytesInRxBuffer;
  
    (void)event;  // Intentionally unreferenced parameter

    while (Hal_UART_RxBufLen(port))
    {
        HalUARTRead (port, &ch, 1); //read one byte

        switch (mspState){
        case MSP_FRAME_PREAMBLE1_STATE:
            if (ch == MSP_PREAMBLE1)
                mspState = MSP_FRAME_PREAMBLE2_STATE;
            break;
        ...
        case MSP_FRAME_DATALEN_HIGH_STATE:
            ...
            pMsg = (mtOSALSerialData_t *)osal_msg_allocate( sizeof ( mtOSALSerialData_t ) +
                                                           MSP_PDU_COMMAND_LEN + mspLEN_Token +   MSP_FRAME_CRC_LEN);// command + data + crc

            if (pMsg){
                /* Fill up what we can */
                pMsg->hdr.event = CMD_SERIAL_MSG;
                pMsg->msg = (uint8*)(pMsg + 1);
                mspLEN_Token += MSP_PDU_COMMAND_LEN + MSP_FRAME_CRC_LEN; // command + data + crc
                mspState = MSP_FRAME_REMAINPDU_STATE;
            }
            else{
                mspState = MSP_FRAME_PREAMBLE1_STATE;
                return;
            }
            break;
        ...
        case MSP_FRAME_REMAINPDU_STATE:
            ...
            pMsg->msg[msptempDataLen++] = ch;

            /* Check number of bytes left in the Rx buffer */
            bytesInRxBuffer = Hal_UART_RxBufLen(port);

            /* If the remain of the data is there, read them all, otherwise, just read enough */
            if (bytesInRxBuffer <= (mspLEN_Token - msptempDataLen)){
                if(bytesInRxBuffer != 0){
                    HalUARTRead (port, &pMsg->msg[msptempDataLen], bytesInRxBuffer);
                    msptempDataLen += bytesInRxBuffer;
                }
            }
            else{
                HalUARTRead (port, &pMsg->msg[msptempDataLen], mspLEN_Token - msptempDataLen);
                msptempDataLen += mspLEN_Token - msptempDataLen;
            }

            /* If number of bytes read is equal to data length, time to move on to FCS */
            if ( msptempDataLen == mspLEN_Token ){
                /* Make sure it's correct */

                if ((mspcheckCRC16((uint8_t*)&pMsg->msg[0], mspLEN_Token) == 0)) {
