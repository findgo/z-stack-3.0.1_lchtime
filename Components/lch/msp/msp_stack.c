...
uint8_t mspcheckFCS( uint8_t *pucFrame, uint16_t len )
{
    ...
    for ( i = 0; i < len; i++, pucFrame++ )
        xorResult = xorResult ^ *pucFrame;
...
{   
    buf->data = buf->buf + MSP_FRAME_HEAD_LEN + MSP_FRAME_DATALEN_LEN + MSP_PDU_COMMAND_LEN;
    buf->data_len = 0;
}
/**
  * @brief   
  ...
  * @retval 
  */
void mspbuf_push(mspbuf_t *buf, uint8_t len)
{
